...
- **플러그인 핫 리로드** - `/reload <플러그인명>` 명령으로 서버 재시작 없이 런타임에 플러그인을 리로드합니다. 이벤트 리스너, 명령어, 권한까지 완전히 언로드/리로드됩니다. 리로드 시 해당 플러그인에 의존하는 플러그인들을 의존 순서대로 자동으로 껐다 켜서(cycle) 새 인스턴스에 다시 바인딩되게 하며, 실패하면 직전에 동작하던 버전으로 롤백합니다. 버전드 네임스페이스 기반 클래스 캐시 무효화(`ClassCacheInvalidator`)와 리소스 인덱스(`PluginResourceIndex` / `PluginResources`)로 역의존성 맵을 포함한 플러그인 상태를 추적·재구성합니다.
  - **제약:** **디렉터리/소스** 플러그인만 리로드 가능합니다. `.phar` 및 단일 파일 스크립트 플러그인은 명확한 메시지와 함께 거부되며 전체 재시작이 필요합니다. PHP는 클래스를 언로드할 수 없으므로, 변경된 플러그인을 리로드할 때마다 이전 클래스 세대가 메모리에 남아 세션 내 리로드 횟수에 비례해 메모리가 증가합니다 — 회수하려면 재시작하세요. 리로드된 플러그인의 클래스를 정적 타입힌트나 `instanceof`로 참조하는 의존 플러그인은, 그 플러그인의 공개 API가 바뀐 경우 함께 리로드(또는 서버 재시작)해야 합니다.
