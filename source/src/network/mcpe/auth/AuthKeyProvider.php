...
class AuthKeyProvider{
		...
		try{
			...
			if($keys === null){
				...
				if(count($keys) === 0){
					...
					$keyCount = count($keys);
					$keyIds = implode(', ', array_keys($pemKeys));
					$this->logger->debug("Successfully fetched {$keyCount} new authentication keys from issuer {$issuer}, key IDs: {$keyIds}");
	...
	private function fetchKeys() : Promise{
		...
		$this->logger->debug("Fetching new authentication keys");
